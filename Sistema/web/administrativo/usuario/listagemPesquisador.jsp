...
<div class="panel-group ">
    <div class="panel panel-default ">
        <div class="panel-heading" >
            <h3 class="panel-title">Listagem de Pesquisadores</h3>
        </div>
        <p></p>
        <p></p>
        <div class="panel-body">
            ...
            <div class="panel panel-default">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <c:forEach items="${pesquisadores}" var="usuario">
                            <tr>
                                <td><c:out value="${pesquisadores.id}"/></td>
                                <td><c:out value="${pesquisadores.nome}"/></td>
                                <td><c:out value="${pesquisadores.email}"/></td>
                                <td><c:out value="${pesquisadores.tipo.descricao}"/></td>
                                <td></td>
                                <td width="10px"><a href="javascript:confirmarMudancaSenha(<c:out value="${pesquisadores.id}"/>,'<c:out value="${pesquisadores.nome}"/>');"><span class="glyphicon glyphicon-lock" title="Mudar Senha"  aria-hidden="true"></span></a></td>
                                <td width="10px"><a href="javascript:confirmarExclusao(<c:out value="${pesquisadores.id}"/>,'<c:out value="${pesquisadores.nome}"/>');"><span class="glyphicon glyphicon-trash" style="color:red" title="Excluir"  aria-hidden="true"></span></a></td>
                                <td width="10px"><a href="cadastroUsuario.do?idLoad=<c:out value="${pesquisadores.id}"/>" label= "Editar" ><span class="glyphicon glyphicon-pencil" title="Editar" aria-hidden="true"/></a></span</td>
                            </tr>
                        </c:forEach>
                        <tr>
                            <td></td>
            ...
            <nav>
                ...
                <ul class="pagination">
                    ...
                    %><li <%=active%> ><a href="listagemPesquisador.do?curPag=<%=i%>&nome=<%=nome%>"><%=i%></a></li><%
                        }
                        %>
                    ...
                    <%
                        ...
                        if (curPag == qtdPag) {
                            ...
                            nextLink = "listagemPesquisador.do?curPag=";
                            nextLink += curPag + 1;
                            nextLink += "&nome=";
                            nextLink += nome;
                        }
...
<script>
    function confirmarMudancaSenha(id, nome) {
        ...
        $('#modal-text').html("Deseja mudar a senha para o pesquisador " + nome + "?");
    ...
    function mudarSenha(id) {
        $(location).attr('href', "listagemPesquisador.do?idMudarSenha=" + id);
    }
    ...
    function excluir(id) {
        $(location).attr('href', "listagemPesquisador.do?idDelete=" + id);
    }
</script>
